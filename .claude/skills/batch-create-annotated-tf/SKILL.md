...
### 実装パターン（run_in_background + TaskOutput）
```
...
#### 6.2 並列処理の実行（run_in_background + TaskOutput パターン）

**並列処理のルール（Claudeへの指示）:**

1. リソースリストを8件ずつのバッチに分割する
2. **Phase 1（起動）**: 8件のTaskを `run_in_background: true` で**1メッセージで同時に**起動し、task_idを記録
3. **Phase 2（結果取得）**: 各task_idに対して `TaskOutput` を呼び出し、結果を取得
4. タイムアウトまたはエラーのTaskは失敗として記録
5. 全リソースが処理されるまでバッチ処理を繰り返す
...
=== バッチ1: [r1, r2, r3, r4, r5, r6, r7, r8] ===

[Phase 1: 起動] 1メッセージで8つのTask呼び出し（run_in_background: true）
  → 即座にtask_idが返る: [id1, id2, id3, id4, id5, id6, id7, id8]
  → 8つのTaskはバックグラウンドで並列実行開始

[Phase 2: 結果取得] 各task_idに対してTaskOutputを呼び出し
  → TaskOutput(id1, block=true, timeout=300000) → 結果またはタイムアウト
  → TaskOutput(id2, block=true, timeout=300000) → 結果またはタイムアウト
  → ... (8件すべて)

[結果検証] ファイル存在確認 + 成功/失敗カウント
...
各バッチのPhase 2完了後、生成されたファイルの存在を確認：

```bash
for resource in BATCH_RESOURCES; do
    if [[ -f "terraform-template/${resource}.tf" ]]; then
        echo "✅ ${resource}"
...
失敗したリソースがある場合は `tmp/{version}/failed_resources.json` に記録:
...
- **失敗リソース:** タイムアウト・エラーのリソースは `tmp/{version}/failed_resources.json` に記録
