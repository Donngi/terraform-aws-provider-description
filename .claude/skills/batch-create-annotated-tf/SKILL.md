...
このスキルは `run_in_background=true` でバックグラウンドエージェントを起動し、各エージェントがファイル書き込みを行う。サンドボックスモードではバックグラウンドエージェントのファイル書き込みが制限されるため、以下のいずれかの方法で実行する必要がある:

```bash
# 方法1: 起動時にサンドボックスを無効化（推奨）
claude --dangerously-skip-permissions

# 方法2: 対話的に権限を許可
# 実行中に書き込み権限のプロンプトが表示された場合は許可する
```

**注意:** サンドボックス無効化は全ての書き込み操作に対する保護を解除するため、信頼できる環境でのみ使用すること。
...
- **prompt**: `"/terraform-aws-annotated-reference {resource_name} provider_version={provider version}"`
...
バックグラウンドで実行されるサブエージェント（`/terraform-aws-annotated-reference`）は以下のいずれかに出力:
- 一次出力先: `.local/terraform-aws-annotated-reference/{version}/{resource}.tf`
- または直接: `terraform-template/{resource}.tf`

親エージェントは Phase 2 で以下を行う:
1. サブエージェント出力ファイル（一次出力先）の存在確認
2. ファイルサイズが0より大きいことを確認（`-s` オプション）
3. 確認後、`terraform-template/{resource}.tf` へコピー
4. 一次出力先になければ、最終出力先を直接確認
5. 成功したリソースを `completed_resources.txt` に記録

**空ファイルの除外:**
ファイルサイズが0バイトの場合は失敗として扱う。これにより、途中で中断されたテンプレート生成を検出できる。

各TaskOutputの結果取得後、生成されたファイルの存在を確認し、即座にチェックポイント：
...
for {resource, task_id} in TASK_IDS; do
    ...
    # サブエージェントの出力先を確認（terraform-aws-annotated-referenceの出力先）
    SOURCE_FILE=".local/terraform-aws-annotated-reference/${VERSION}/${resource}.tf"
    DEST_FILE="terraform-template/${resource}.tf"

    # ファイル存在 + サイズ > 0 を確認
    if [[ -f "$SOURCE_FILE" ]] && [[ -s "$SOURCE_FILE" ]]; then
        # 成功: ファイルをコピー（既存ファイルは上書き）
        cp "$SOURCE_FILE" "$DEST_FILE"
        echo "✅ ${resource}"
        SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
        echo "${resource}" >> "$COMPLETED_FILE"
    elif [[ -f "$DEST_FILE" ]] && [[ -s "$DEST_FILE" ]]; then
        # サブエージェントが直接terraform-template/に出力した場合
        echo "✅ ${resource} (direct output)"
        SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
        echo "${resource}" >> "$COMPLETED_FILE"
    else
        echo "❌ ${resource} (empty or not found)"
...
- **空ファイル（サイズ0）は失敗として扱う**（`-s` オプションで検出）
