...
task_result = Task(
    ...
    prompt="/terraform-aws-annotated-reference {resource_name} provider_version={version} output_mode=content",
    max_turns=50,
    run_in_background=true
)

# 返り値からoutput_fileとtask_idを記録
TASK_IDS.append({
...
**重要:** `output_file`はTaskの出力が自動保存されるファイルパスです。Phase 2でこのパスをシェルスクリプトに渡します。
...
**Phase 1で取得した`output_file`パスを使用:**

各Taskは`run_in_background=true`で起動されており、**出力は自動的にファイルに保存**されています。
Task起動時に返された`output_file`パスを記録しておきます。

**Phase 2での処理:**

```
for {resource, task_id, output_file} in TASK_IDS:
    # 1. TaskOutputで完了を待機（結果はコンテキストに含めない）
    TaskOutput(task_id, block=true, timeout=300000)

    # 2. シェルスクリプトでJSON抽出・ファイル書き込み
    Bash("lib/extract_and_write.sh ${output_file} terraform-template/${resource}.tf ${resource}")

    # 3. 終了コードで判定
    if exit_code == 0:
        echo "✅ ${resource}"
        SUCCESS_COUNT++
        echo "${resource}" >> ".local/${VERSION}/completed_resources.txt"
    else:
        echo "❌ ${resource} - Exit code: ${exit_code}"
        # 終了コードから失敗理由を判定
        case exit_code:
            1: reason = "no_json_output"
            2: reason = "invalid_json"
            3: reason = "subagent_error"
            4: reason = "unknown_status"
            5: reason = "empty_content"
            6: reason = "write_failed"
            default: reason = "unknown_error"

        FAILED_RESOURCES.append({"resource": resource, "reason": reason})
```
...
```
    ...
    FAILED_RESOURCES.append({"resource": resource, "reason": "timeout"})
    continue
```

**重要:**
- **TaskOutputの巨大な結果は親エージェントのコンテキストに含まれない**
- シェルスクリプトが`output_file`から直接抽出・書き込み
- 親エージェントには終了コード（1バイト）のみが返る
...
{
  "failed": [
    {"resource": "aws_x", "reason": "no_json_output"},
    {"resource": "aws_y", "reason": "invalid_json"},
    {"resource": "aws_z", "reason": "subagent_error"},
    {"resource": "aws_w", "reason": "empty_content"},
    {"resource": "aws_v", "reason": "unknown_status"},
    {"resource": "aws_u", "reason": "timeout"},
    {"resource": "aws_t", "reason": "write_failed"}
  ]
}
```

**失敗理由の分類:**
- `timeout`: TaskOutputタイムアウト
- `no_json_output`: JSON未発見
- `invalid_json`: JSONパースエラー
- `subagent_error`: サブエージェントエラー
- `empty_content`: contentが空
- `unknown_status`: 不明なstatus
- `write_failed`: ファイル書き込み失敗
