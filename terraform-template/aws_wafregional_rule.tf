...
#---------------------------------------------------------------
#
# AWS WAF Regional ルールをプロビジョニングするリソースです。
# ルールは、Webリクエストを検査するための条件（Predicate）のセットを定義します。
...
#   - Predicate API Reference: https://docs.aws.amazon.com/waf/latest/APIReference/API_wafRegional_Predicate.html
#
# Terraform Registry:
...
resource "aws_wafregional_rule" "example" {
  ...
  # 設定内容: ルールのAmazon CloudWatchメトリクス用の名前を指定します。
  ...
  # predicate (Optional)
  # 設定内容: ルールに含める条件オブジェクトのセットを指定します。
  # 注意: 複数のpredicateブロックを指定可能です。
  #       すべての条件がAND条件で評価されます。
  #       条件が空のルールはトラフィックの検査・制御を行いません。
  ...
  #   条件オブジェクトをルールに追加し、マッチングを行います。
  #   - https://docs.aws.amazon.com/waf/latest/APIReference/API_wafRegional_Predicate.html
  predicate {
    # data_id (Required)
...
#
# - id: WAF Regional ルールのID
#
# - arn: WAF Regional ルールのAmazon Resource Name (ARN)
#
