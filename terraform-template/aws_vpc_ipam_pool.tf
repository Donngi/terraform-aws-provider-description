...
resource "aws_vpc_ipam_pool" "example" {
  ...
  # 設定可能な値:
  #   - true: パブリックにアドバタイズする
  #   - false: パブリックにアドバタイズしない（デフォルト）
  # 省略時: false
  # 注意: この引数は、address_family = "ipv6" かつ public_ip_source = "byoip" の場合に必須です。
  #       IPv4プールスペースまたはpublic_ip_source = "amazon"の場合は利用できません。
  ...
  # 省略時: tagsとprovider default_tagsの結合
  # 注意: この属性はComputedであり、通常は明示的に設定する必要はありません。
