...
#---------------------------------------------------------------
#
# AWS WAF Regional の正規表現マッチセットをプロビジョニングするリソースです。
# 正規表現マッチセットは、Webリクエストの特定部分（ヘッダー、URI等）に対して
# 正規表現パターンでマッチングを行う条件を定義します。
...
#   - TextTransformation: https://docs.aws.amazon.com/waf/latest/APIReference/API_waf_ByteMatchTuple.html
#
# Terraform Registry:
...
resource "aws_wafregional_regex_match_set" "example" {
  ...
  # name (Required)
  # 設定内容: 正規表現マッチセットの名前または説明を指定します。
  # 設定可能な値: 1-128文字の文字列
  # 注意: 作成後に変更することはできません。
  name = "example-regex-match-set"
  ...
  # 設定内容: AWS WAFがWebリクエスト内で検索する正規表現パターン、
  #           検索対象の場所、およびその他の設定を指定します。
  # 注意: 複数のregex_match_tupleブロックを定義可能です。
  regex_match_tuple {

    # regex_pattern_set_id (Required)
    # 設定内容: マッチングに使用する正規表現パターンセットのIDを指定します。
    # 設定可能な値: aws_wafregional_regex_pattern_set リソースのID
    # 関連リソース: aws_wafregional_regex_pattern_set
    #   正規表現パターンの集合を定義するリソース。事前に作成が必要です。
    ...
    # 注意: 1つのregex_match_tupleにつき1つのfield_to_matchブロックが必須です。
    field_to_match {
...
resource "aws_wafregional_regex_pattern_set" "example" {
  ...
  regex_pattern_strings = ["BadBot", "EvilCrawler"]
}
