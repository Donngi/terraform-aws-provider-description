...
#---------------------------------------------------------------
#
# AWS WAF Regional のサイズ制約セットをプロビジョニングするリソースです。
# サイズ制約セットは、Webリクエストの特定部分（ヘッダー、ボディ等）のサイズを
# 検査し、指定したサイズ条件に基づいてマッチングを行う条件を定義します。
...
resource "aws_wafregional_size_constraint_set" "example" {
  ...
  # 設定可能な値: 1-128文字の文字列
  # 注意: 作成後に変更することはできません。
  name = "example-size-constraint-set"
  ...
  # 設定内容: AWS WAFがWebリクエスト内で検索するサイズ条件、
  #           検索対象の場所、およびその他の設定を指定します。
  # 注意: 複数のsize_constraintsブロックを定義可能です。
  size_constraints {

    # comparison_operator (Required)
    # 設定内容: AWS WAFがsize（バイト数）とWebリクエストのフィールドのサイズを
    #           比較するために使用する演算子を指定します。
    # 設定可能な値:
    #   - "EQ": sizeと等しい場合にマッチ
    #   - "NE": sizeと等しくない場合にマッチ
    #   - "LE": size以下の場合にマッチ
    ...
    #   - "GE": size以上の場合にマッチ
    #   - "GT": sizeより大きい場合にマッチ
    # 参考: https://docs.aws.amazon.com/waf/latest/APIReference/API_waf_SizeConstraint.html
    comparison_operator = "GT"

    # size (Required)
    # 設定内容: comparison_operatorと組み合わせて使用するバイト数を指定します。
    # 設定可能な値: 0以上の整数（バイト単位）
    # 注意: BODYフィールドの場合、AWS WAFは最初の8192バイトのみ検査します。
    size = 4096

    # text_transformation (Required)
    # 設定内容: 攻撃者がAWS WAFをバイパスするために使用する異常なフォーマットを
    #           排除するためのテキスト変換を指定します。
    #           変換後のリクエストのサイズが実際のマッチングに使用されます。
    # 設定可能な値:
    #   - "NONE": テキスト変換を行わない
    ...
    # 設定内容: AWS WAFがサイズを検査するWebリクエストの部分を指定します。
    # 注意: 1つのsize_constraintsにつき1つのfield_to_matchブロックが必須です。
    field_to_match {
      ...
      # data (Optional)
      # 設定内容: typeがHEADERの場合は検査対象のヘッダー名、
      #           typeがSINGLE_QUERY_ARGの場合は検査対象のパラメータ名を指定します。
...
#
# - id: WAF Regional サイズ制約セットのID
# - arn: WAF Regional サイズ制約セットのARN
#
#---------------------------------------------------------------
